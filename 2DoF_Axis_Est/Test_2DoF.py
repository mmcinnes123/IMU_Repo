...
def get_J1_J2_from_opt(subject_code, IMU_type_for_opt, trial_for_opt, opt_method, subject_time_dict, sample_rate, debug):
    ...
    def get_variation(IMU_quats):
        quat_changes = np.zeros((len(IMU_quats), 4))
        q0 = IMU_quats[0]
        for row in range(len(IMU_quats)):
            diff = qmt.qmult(qmt.qinv(IMU_quats[row]), q0)
            quat_changes[row] = diff
        angles = qmt.quatAngle(quat_changes, plot=True)
        angles = abs(angles)
        mean_diff = np.mean(np.array(angles))
        return mean_diff

    IMU1_mean_diff = get_variation(IMU2_trimmed)
    IMU2_mean_diff = get_variation(IMU3_trimmed)
    print('Mean pairwise variation in orientation IMU1: ', IMU1_mean_diff * 180 / np.pi)
    print('Mean pairwise variation in orientation IMU2: ', IMU2_mean_diff * 180 / np.pi)
